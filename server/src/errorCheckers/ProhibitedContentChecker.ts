...
function normalizeText(text: string): string {
  ...
  function getContext(text: string, index: number, length: number, contextSize: number = 20): string {
    const start = Math.max(0, index - contextSize);
    const end = Math.min(text.length, index + length + contextSize);
    return text.slice(start, end);
  }
  
  export function checkMonitoredPharmacyWords(item: FeedItem): ErrorResult | null {
    ...
    const foundWords: Array<{ word: string, field: 'title' | 'description', context: string }> = [];
    ...
    if (foundWords.length > 0) {
      const examples = foundWords.slice(0, 3).map(({ word, field, context }) => 
        `"${context}" (found: "${word}" in ${field})`
      );
